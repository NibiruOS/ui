...
public class IOSCheckBox
    ...
    public IOSCheckBox() {
        ...
        check.addTargetActionForControlEvents((UISwitch check, long flags) -> getValue().notifyObservers(), UIControlEvents.ValueChanged);
        label.setUserInteractionEnabled(true);
        TouchUpInsideHandlerRegistration.alloc().initWithControlAndClickHandler(label, () -> getValue().set(!getValue().get()));
