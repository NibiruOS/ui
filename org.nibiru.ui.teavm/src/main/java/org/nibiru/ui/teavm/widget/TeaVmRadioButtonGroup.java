...
public class TeaVmRadioButtonGroup<V> extends TeaVmValueWidget<HTMLElement, V> implements RadioButtonGroup<V> {
	...
	public TeaVmRadioButtonGroup() {
		this(createElement("div"));
	}

	public TeaVmRadioButtonGroup(HTMLElement control) {
		super(control);
		groupId = groupCount++;
	}
		...
		if (items == null) {
			items = new BaseValue<Iterable<V>>() {
				...
				protected void setValue(Iterable<V> value) {
					this.value = value;
					while (control.getChildNodes().getLength() > 0) {
						control.removeChild(control.getChildNodes().item(0));
					}
